...
def meta_test(eps, eval_loader, learner_w_grad, learner_wo_grad, metalearner, args):
    total_acc = 0
    n = 0
    for subeps, (episode_x, episode_y) in enumerate(eval_loader):#enumerate(tqdm(eval_loader, ascii=True)):
...
def main():
    ...
    metalearner = MetaLearner(args.input_size, args.hidden_size, learner_w_grad.get_flat_params().size(0), use_qrnn).to(args.dev)
    ...
    for eps, (episode_x, episode_y) in enumerate(train_loader):
        ...
        if((eps + 1) % 250 == 0 or eps == 0):
            print(eps + 1, "/", args.episode," Loss: ", loss.item(), " Acc:", acc)
        #logger.batch_info(eps=eps, totaleps=args.episode, loss=loss.item(), acc=acc, phase='train')

        # Meta-validation
        if ((eps + 1) % args.val_freq == 0 and eps != 0) or eps + 1 == args.episode:
